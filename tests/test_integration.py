# -*- coding: utf-8 -*-
"""
Интеграционные тесты SMP12C VibroDiag Analyzer v1.4
=====================================================
...
# =============================================================================
# TEST 2.9: Sensor Serial (record_number) Analysis
# =============================================================================

@pytest.mark.asyncio
class TestSensorSerialAnalysis:
    """Test sensor_serial (record_number) field analysis.
    
    Analysis performed 2026-05-21 on WTG37 test data (24 .rd2 files):
    - All 24 files have SAME sensor_serial = 38408
    - sensor_serial = record_number is a DAILY RECORD NUMBER for the turbine
    - All .rd2 files in one ZIP archive share the same value
    
    Conclusion: sensor_serial is NOT unique and CANNOT be used in the unique key.
    It is stored as an informational field for auditing and linking files.
    """
    ...
    def test_sensor_serial_not_globally_unique(self, test_storage_path):
        """Test that sensor_serial is NOT globally unique (informational only)."""
        from smp12c_vibrodiag.parsers.rd2_parser import RD2Parser
        from collections import Counter

        rd2_files = list(test_storage_path.rglob("*.rd2"))
        if not rd2_files:
            pytest.skip("No .rd2 files found")
        
        serials = []
        for f in rd2_files[:100]:  # Sample up to 100 files
            try:
                parser = RD2Parser(str(f))
                data = parser.parse()
                serials.append(data['metadata']['sensor_serial'])
            except:
                pass
        
        if not serials:
            pytest.skip("No valid sensor_serial found")
        
        unique_count = len(set(serials))
        total_count = len(serials)
        
        # As of 2026-05-21 analysis: NOT globally unique
        # All files in one archive share the same sensor_serial
        assert unique_count < total_count, (
            f"Expected sensor_serial to NOT be globally unique "
            f"(got {unique_count}/{total_count} unique). "
            "This is expected behavior - it's a daily record number."
        )
