...
## Поле record_number в .rd2 файлах (v1.4)

### Что это такое

Первое поле строки 1 .rd2 файла (`record_number`) — это **порядковый номер записи за сутки** для конкретной ВЭУ. Все .rd2 файлы внутри одного ZIP-архива (8 датчиков x 3 фильтра = 24 файла) имеют **одинаковый** `record_number`.

**Пример:**
```
W1436 WTG37 SMP_20250901_38408_SENSOR_01_LOW_W.rd2   → record_number=38408
W1436 WTG37 SMP_20250901_38408_SENSOR_01_HIGH_W.rd2  → record_number=38408
W1436 WTG37 SMP_20250901_38408_SENSOR_02_LOW_W.rd2   → record_number=38408
... (все 24 файла архива)                             → record_number=38408
```

### Выводы анализа (выполнен 2026-05-21)

| Критерий уникальности | Результат |
|----------------------|-----------|
| Глобальная уникальность | ❌ Нет — 24 файла одного архива имеют одинаковое значение |
| Уникальность в пределах датчика | ❌ Нет |
| Уникальность в пределах (датчик + фильтр) | ❌ Нет |
| Уникальность в пределах одного архива | ✅ Да — все файлы архива имеют одинаковый номер |

### Решение

`sensor_serial` (алиас `record_number`) **НЕ используется** в составном уникальном ключе. Оставлено как **информационное поле** для:
- Связи всех .rd2 файлов одного архива
- Отладки и аудита
- Возможного будущего анализа (если формат изменится)
...
```python
# В парсере: извлекаем как record_number и sensor_serial (алиасы)
self.metadata['record_number'] = line1[0]
self.metadata['sensor_serial'] = line1[0]  # информационное поле

# В БД: сохраняем, но не используем для дедупликации
archive.sensor_serial = metadata.get('sensor_serial')  # информационно
```
