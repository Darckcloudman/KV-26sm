...
### Поле record_number (sensor_serial) в .rd2 файлах (v1.4)

**Анализ выполнен:** 2026-05-21 на тестовых данных WTG37 (24 файла)

**Результат:** Первое поле строки 1 (`record_number` = `sensor_serial`) — это **порядковый номер записи за сутки** для конкретной ВЭУ. Все 24 .rd2 файла внутри одного ZIP-архива имеют **одинаковое значение** (например, `38408`).

| Критерий уникальности | Результат |
|----------------------|-----------|
| Глобальная уникальность | ❌ Нет — 24 файла архива = 1 значение |
| Уникальность в пределах датчика | ❌ Нет |
| Уникальность в пределах (датчик + фильтр) | ❌ Нет |
| Уникальность в пределах архива | ✅ Да — общий номер для всех файлов |

**Решение:** `sensor_serial` **НЕ входит** в составной уникальный ключ. Оставлено как **информационное поле** для аудита и связи файлов одного архива.

**Уникальный ключ остаётся:**
```sql
UNIQUE(turbine_id, record_datetime, sensor_id, filter_type)
```

**Модель Archive (обновлена):**
- Добавлено поле `sensor_serial VARCHAR(50) NULL` — информационное
- Заполняется из первого поля строки 1 .rd2 файла (record_number)
...
- Выводит рекомендацию по использованию
...
## 📅 Сессия 2025-06-02 — v1.4.1 UI/UX доработки настроек, Type Checking

### Диалог настроек (settings_dialog.py)

#### Checkbox стили
- Упрощены до стандартных Qt checkbox с кастомными цветами
- Размер индикатора: 10×10 px
- Неотмеченный: фон `#3A3A3A`, отмеченный: фон `#FFFFFF` с чёрной галочкой (PNG)
- Сгенерирован `checkbox_checked.png` (10×10 прозрачный PNG)

#### Вкладка "Хранилище" (полный редизайн)
- **Структура** (сверху вниз):
  1. Лог сканирования (растянут) + кнопка "Сохранить лог" справа
  2. Путь к архивам + кнопка "Обзор..."
  3. Кнопки "Сканировать" / "Отмена" + прогресс-бар
  4. Кнопка "Добавить в список" (активна после сканирования)
- **Кнопка "Сканировать"** (переименована с "Сканировать хранилище"):
  - Иконка: `mdi.folder-search` (чёрная, scale 0.8)
  - Стиль: белый фон, чёрный текст
- **Кнопка "Отмена"**:
  - Иконка: `mdi.cancel` (чёрная, scale 0.8)
  - Активный стиль: белый фон `#FFFFFF` (как у остальных кнопок)
  - Неактивный: серый фон
- **Кнопка "Обзор..."**:
  - Иконка: `mdi.folder-plus` (чёрная, scale 0.8)
- **Кнопка "Сохранить лог"**:
  - Иконка: `fa5.save` (чёрная, scale 0.8)
- **Кнопка "Добавить в список"** (новая):
  - Иконка: `ri.file-add-fill` (чёрная, scale 0.8)
  - Активируется после завершения или отмены сканирования (если найдены архивы)
  - По нажатию: передаёт архивы в таблицу HomeScreen, закрывает диалог настроек, переключает на вкладку Home

#### Вкладка "Логирование" (реструктуризация)
- Кнопки "Обновить", "Очистить", "Сохранить" перенесены **под** окно с логом
- Кнопки прижаты к правому краю
- **Иконки QtAwesome**:
  - Обновить: `mdi.refresh`
  - Очистить: `mdi.delete-empty`
  - Сохранить: `fa5.save`

#### Кнопки "Применить" / "Отмена"
- Убраны из общего подвала диалога
- Оставлены **только** во вкладке "База данных"
- В остальных вкладках (Хранилище, Логирование, Модули) отсутствуют

#### Передача архивов в HomeScreen
- Новый сигнал `archives_found = Signal(list)` в SettingsDialog
- Новый сигнал `switch_to_home = Signal()` для переключения вкладки
- Сканирование собирает архивы в `self._found_archives: list`
- Метод `_add_to_list()` — отправляет архивы, эмитит сигналы, закрывает диалог
- `HomeScreen.add_archives()` — добавляет архивы в таблицу:
  - Проверка дубликатов по пути
  - Сортировка по имени файла
  - Установка директории из первого архива

### Type Checking исправления

#### TrendsScreen / UploadInfoScreen
- **Причина:** `self.repository_switcher.repository` возвращает `Optional[IVibrationRepository]` (может быть `None` в файловом режиме)
- **Исправление:**
  - `TrendsScreen.__init__(repository: Optional[IVibrationRepository] = None)`
  - `UploadInfoScreen.__init__(repository: Optional[IVibrationRepository] = None)`
  - Проверка `if self.repository is None: return` перед созданием потоков `TurbinesLoaderThread` и `TrendsWorker`

#### UploadInfoScreen.set_upload_data
- **Причина:** `sensor_files: Dict[int, Dict[str, str]] = None` — `None` не совместим с `Dict`
- **Исправление:** `sensor_files: Optional[Dict[int, Dict[str, str]]] = None`

#### UploadInfoScreen._update_display
- **Причина:** `widget = self.sensors_layout.itemAt(i).widget()` — `widget()` возвращает `Optional[QWidget]`, `None` не имеет `deleteLater()`
- **Исправление:** промежуточная переменная с проверкой `if widget is not None`
...
├── kwf_prometheus/
│   ├── gui/
│   │   ├── settings_dialog.py              ← Обновлён: UI редизайн, сигналы, сканирование
│   │   ├── home_screen.py                  ← Обновлён: add_archives(), без status_label
│   │   ├── main_window.py                  ← Обновлён: switch_to_home.connect()
│   │   ├── trends_screen.py                ← Обновлён: Optional[IVibrationRepository]
│   │   ├── upload_info_screen.py           ← Обновлён: Optional типы, widget проверки
│   │   ├── ui_styles.py                    ← Обновлён: CHECKBOX_STYLE
│   │   └── checkbox_checked.png            ← Новый: 10×10 PNG галочка
└── CHANGELOG_v1.3.md                       ← Обновлён: раздел v1.4.1
```

### Меры от повторного возникновения type errors
- Всегда использовать `Optional[...]` для параметров со значением по умолчанию `None`
- Перед передачей `Optional[T]` в функции, ожидающие `T`, делать проверку на `None`
- Для `item.widget()` всегда сохранять в переменную и проверять перед использованием
- При изменении типа атрибута класса проверять все точки использования
