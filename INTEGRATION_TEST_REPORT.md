# Отчёт о тестировании интеграции KWF Prometheus v1.4
...
## 7. Анализ sensor_serial (record_number) — ВЫПОЛНЕН

**Дата:** 2026-05-21
**Данные:** 24 .rd2 файла WTG37 (8 датчиков x 3 фильтра)

### Результат

| Критерий | Результат |
|----------|-----------|
| Глобальная уникальность | ❌ Нет — все 24 файла имеют `sensor_serial = 38408` |
| Уникальность в пределах датчика | ❌ Нет |
| Уникальность (датчик + фильтр) | ❌ Нет |
| Уникальность в пределах архива | ✅ Да — общий номер для всех файлов |

### Вывод

`sensor_serial` (первое поле строки 1 .rd2) — это **порядковый номер записи за сутки** для конкретной ВЭУ. Все .rd2 файлы внутри одного ZIP-архива имеют **одинаковое значение**.

**Решение:** `sensor_serial` **НЕ входит** в составной уникальный ключ. Оставлено как **информационное поле** (аудит, связь файлов одного архива).
...
## 8. Рекомендации по дальнейшему тестированию на Кольской ВЭС
...
4. **Настроить `AUTO_SCAN_INTERVAL_MINUTES = 60`** для production.
